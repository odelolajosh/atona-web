import { ChatMessage, ChatState, Conversation, ConversationId, GroupedMessages, IStorage, MessageContentType, MessageGroup, Participant, Presence, User, UserId } from "@chatscope/use-chat";
...
export interface ChatStorageParams {
  groupIdGenerator: GroupIdGenerator;
  messageIdGenerator?: MessageIdGenerator;
}

export class ChatStorage<ConversationData = unknown>
   ...
   */
  constructor({ groupIdGenerator, messageIdGenerator }: ChatStorageParams) {
  ...
  private getMessageWithId(
    ...
    if (generateId) {
      if (!this.messageIdGenerator) {
        ...
        message.id = this.messageIdGenerator(message);
        return message;
  ...
  addMessage(
    ...
    if (conversationId in this.messages) {
      ...
      if (
        ...
        lastGroup.messages[0].createdTime.toDateString() === newMessage.createdTime.toDateString()) {
